...
public class MeshGenerator : MonoBehaviour {
	...
	public void GenerateMesh(int[,] map, float squareSize) {
		...
		for (int i = 0; i < vertices.Count; i++) {
			float percentX = Mathf.InverseLerp(0, map.GetLength(0) * squareSize, vertices[i].x) * textureTileAmount;
			float percentY = Mathf.InverseLerp(0, map.GetLength(0) * squareSize, vertices[i].y) * textureTileAmount;
		...
		// collider
		PolygonCollider2D[] currentColliders = GetComponents<PolygonCollider2D>();
		foreach (PolygonCollider2D collider in currentColliders) {
			Destroy(collider);
		}
		foreach (List<int> outline in outlines) {
			List<Vector2> points = new List<Vector2>();
			foreach (int index in outline) {
				points.Add(vertices[index]);
			}
			PolygonCollider2D pol = gameObject.AddComponent(typeof(PolygonCollider2D)) as PolygonCollider2D;
			pol.points = points.ToArray();
		}
	}
	...
	void CalculateMeshOutlines() {
		for (int vertexIndex = 0; vertexIndex < vertices.Count; vertexIndex++) {
			if (!checkedVertices.Contains(vertexIndex)) {
				int newOutlineVertex = GetConnectectedOutlineVertex(vertexIndex);
	...
	void FollowOutline(int vertexIndex, int outlineIndex) {
		...
		checkedVertices.Add(vertexIndex);
		int nextVertexIndex = GetConnectectedOutlineVertex(vertexIndex);
		if (nextVertexIndex != -1) {
			FollowOutline(nextVertexIndex, outlineIndex);
		}
	}

	int GetConnectectedOutlineVertex(int vertexIndex) {
